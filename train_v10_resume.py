#!/usr/bin/env python3
"""
v10 training RESUME from latest checkpoint (epoch 2).
Same setup: top-2 backbone layers, lr=5e-6, 6 epochs, 200 preferences.
"""
...
logger = logging.getLogger("train_v10_resume")
...
# Save a .bak copy so we can always restore even if the process is killed mid-write
...
trainer = DPOTrainer(
    ...
    checkpoint_dir="/workspace/dpo_checkpoints_v10",
...
# trainer.train() auto-resumes from checkpoint_dir/latest/training_state.pt
try:
    trainer.train()
finally:
    # ALWAYS restore the original trainer.py — try from memory first, fall back to .bak
    try:
        with open(trainer_path, 'w') as f:
            f.write(original_source)
...
logger.info("Checkpoints: /workspace/dpo_checkpoints_v10")
